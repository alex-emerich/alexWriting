...
In this section we will add the data structure to our newly created `people.db` database by creating a `student` table. In order to create a table within your database, you will utilize the `CREATE TABLE` statement with the following syntax:
...
After creating your table, you can verify the structure of your table by using the `.schema` command. For example, if we want to make sure the previously created `student` table is structured the way we intended we can check using the following syntax:
...
In order to drop a table from a database in SQLite, you will use the `DROP TABLE` statement. This statement is used as follows for dropping the `student` table:
